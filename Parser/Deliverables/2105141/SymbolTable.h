...
private:
    string name;
    string type;
    string var_type; // to find semantic errors(type mismatches) in the parser
...
public:
    SymbolInfo(string name, string raw_type, string var_type, string func_ret_type = "", string func_args = "", int func_args_count = 0) {
        this->name = name;
        this->next = nullptr;
        ...
        cout << "SymbolInfo : " << name << ", " << raw_type << ", " << var_type << ", " << func_ret_type << ", " << func_args << ", " << func_args_count << endl;
...
public:
    ...
    bool insert(string name, string type, string var_type, string func_ret_type = "", string func_args = "", int func_args_count = 0) {
        SymbolInfo *symbol = new SymbolInfo(name, type, var_type, func_ret_type, func_args, func_args_count);
    ...
    void testPrint() {
        ...
        for (int i = 0; i < num_buckets; i++) {
            ...
            SymbolInfo *curr = hash_table[i];
            while (curr != nullptr) {
                line += curr->getName() + " : " + curr->getType() + " " + curr->getVarType() + " " + curr->getFuncRetType() + " " + curr->getFuncArgs() + " " + to_string(curr->getFuncArgsCount()) + "\n";
                curr = curr->getNext();
            }
            if (line != to_string(i) + " --> ") {
                cout << line << endl;
...
public:
    ...
    bool insert(string name, string type, string var_type, string func_ret_type = "", string func_args = "", int func_args_count = 0) {
        return current_scope->insert(name, type, var_type, func_ret_type, func_args, func_args_count);
    }
